...
async function createConnectionProfile(orgName = 'org1') {
    try {
        ...
        const peer0TLS = fs.readFileSync(orgConfig.paths.certificates.peer0TLS, 'utf8');
        const peer1TLS = fs.readFileSync(orgConfig.paths.certificates.peer1TLS, 'utf8');
        ...
        const connectionProfile = {
            ...
            "peers": {
                [orgConfig.endpoints.peers.peer0.hostname]: {
                    ...
                    "grpcOptions": {
                        "ssl-target-name-override": orgConfig.endpoints.peers.peer0.hostname,
                ...
                [orgConfig.endpoints.peers.peer1.hostname]: {
                    ...
                    "grpcOptions": {
                        "ssl-target-name-override": orgConfig.endpoints.peers.peer1.hostname,
        ...
        fs.writeFileSync(orgConfig.paths.connectionProfile, JSON.stringify(connectionProfile, null, 2));
        console.log(`Connection profile created for ${orgConfig.name} at:`, orgConfig.paths.connectionProfile);
...
function loadConnectionProfile(orgName = 'org1') {
    try {
        ...
        if (!fs.existsSync(orgConfig.paths.connectionProfile)) {
            throw new Error(`Connection profile for ${orgConfig.name} not found. Please run POST /api/init/${orgName} first to create it.`);
        }
        
        return JSON.parse(fs.readFileSync(orgConfig.paths.connectionProfile, 'utf8'));
    } catch (error) {
...
async function createAllConnectionProfiles() {
    ...
    for (const orgName of orgs) {
        try {
            ...
            console.error(`Failed to create connection profile for ${orgName}:`, error.message);
